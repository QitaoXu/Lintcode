...
class Solution:
    def getSkyline(self, buildings):
        ...
        for position, height, index, is_start in points:
            ...
            self.merge_to(intervals, last_position, position, max_height, results)
            if is_start:
                maxheap.push((height, index))
            else:
                maxheap.remove((height, index))
            last_position = position
        results.append([last_position, 0])
        return results
        
    def merge_to(self, intervals, start, end, height, results):
        if start is None or start == end:
            return
        
        if not intervals:
            intervals.append([start, end, height])
            results.append([start, height])
            return
